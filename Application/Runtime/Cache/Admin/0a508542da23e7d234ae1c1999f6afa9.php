...
var onmol = function(obj){
	var id  = $(obj).attr('data-id');	
	 the_id = $.layer({
	    ...
	    border: [0],
	    iframe: {
	        src: '/admin.php/Cmsmanager/one_edit/id/'+id
	    },
	    title: false,
	    area: ['1000px','600px']
	}); 
...
var AX = function(info){
	...
	case 1:
		layer.msg('单页添加成功',1,{type:1,shade:false,rate:'left'});
		layer.close(the_id);
		ajaxpage();	
		break;
	case 2:
		layer.msg('单页添加失败',1,{type:2,shade:false,rate:'left'});
		layer.close(the_id);
		ajaxpage();
		break;
	case 3:
		layer.msg('单页修改成功',1,{type:1,shade:false,rate:'left'});
		layer.close(the_id);
		ajaxpage();
		break;
	case 4:
		layer.msg('单页修改失败',1,{type:2,shade:false,rate:'left'});
		layer.close(the_id);
		ajaxpage();
		break;
	default:
...
var ajaxpage = function(obj){
	...
	$(function(){
		var url = "/admin.php/Cmsmanager/one_show/do/load/p/"+dp;
...
var ondel = function(id){
	$.layer({
	    ...
	    dialog: {
	        ...
	        btn: ['确定','取消'],
	        yes: function(){
	        		var url = "/admin.php/Cmsmanager/one_del/do/del/id/"+id;
...
var oncat = function(obj){		
 	$.layer({
	    ...
	    border: [0],
	    iframe: {
	        src: "/admin.php/Cmsmanager/one_cat"
	    },
	    title: false,
	    area: ['1200px','600px']
	}); 
...
<div class="dux-bread">
    ...
    <div class="dux-admin">
        <div class="dux-tools">
            <div class="bread-head">单页管理<span class="small">添加单页</span></div>
            <br>
            <div class="tools-function clearfix">
                <div class="float-left">
                   <a onclick="oncat(this)" class="button button-small bg-main icon-list" href="javascript:void(0);">单页分类</a>
                </div>
