...
<#else>
      <#list data.list as list>
        <tr >
          ...
          <td><#if (list.type==1)>中文网站<#elseif (list.type==2)>英文网站<#else>商城网站</#if></td>
          <td>${list.mode}</td>
          <td>${list.link}</td>
          <td>${list.code}</td>
          <td>${list.dateline}</td>
...
var onmol = function(obj){
	var id  = $(obj).attr('data-id');	
	 the_id = $.layer({
	    ...
	    border: [0],
	    iframe: {
	        src: '/admin.php/Cmsmanager/goods_edit/id/'+id
	    },
	    title: false,
	    area: ['1000px','600px']
	}); 
...
var ajaxpage = function(obj){
	...
	$(function(){
		var url = "/admin.php/Cmsmanager/goods_show/do/load/p/"+dp;
...
var search = function(){
	...
	$(function(){
		var url = "/admin.php/Cmsmanager/goods_show?do=search&keysword="+keysword+"&code="+code;
		var the_load = layer.load(30);
		$.get(url,function(data,status){
			ETlist(data['list'],'#nan-list','#theTemplate');
			if(status=='success'){
...
var ondel = function(id){
	$.layer({
	    ...
	    dialog: {
	        ...
	        btn: ['确定','取消'],
	        yes: function(){
	        		var url = "/admin.php/Cmsmanager/goods_del/do/del/id/"+id;
...
var oncat = function(obj){		
 	$.layer({
	    ...
	    border: [0],
	    iframe: {
	        src: "/admin.php/Cmsmanager/goods_cat"
	    },
	    title: false,
	    area: ['1200px','600px']
	}); 
                   ...
                   <a onclick="oncat(this)" class="button button-small bg-main icon-list" href="javascript:void(0);">产品分类</a>
	                ...
	                <div class="float-left distance-left">                    		
	                   		<select class="input" name='code'> 
		                   		<option></option> 
		                   		<?php if(is_array($goods_cat)): foreach($goods_cat as $key=>$vo): ?><option value='<?php echo ($vo["code"]); ?>'><?php echo ($vo["title"]); ?></option><?php endforeach; endif; ?>
	                   		</select>
	                </div>
	                <div class="float-left distance-left"> 
